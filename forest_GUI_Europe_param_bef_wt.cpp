...
#include "forest_GUI_Europe_wt.h"                     // definitions
#include "dima_float.h"
#include "readInput_Europe_ua_wt.cpp" 
#include "defineSpecies.cpp"
#include "harvestWood.cpp"
#include "initManagedForest5_ageStruct_Europe_bef_wt.cpp" // BEF is taken into account
...
//------------------------------------------------------
#include "calc_Europa_2Aug2011_v021_bef.cpp" // The same as calc_Europa_2Aug2011_v021.cpp + affor, defor and FM emissions are estimated taking into account BEF + Area and Biomass of new forests over 20 y.o. are tracked
...
//************************************************
  ...
  string suffix="_WoodTypes_UA";
  string suffix0=suffix;  
double priceC = 0.;
   if (PriceC > 0){ suffix += "_Pco2_" + IntToStr(PriceC);     
                   priceC =  PriceC * 44./12. /* exchangeRate*/;}
   if (PriceC==0) {suffix += "_Pco2_0";}   
   string fileOut = settings.outputPath +"\\" + "details" + suffix + ".txt";

  fff.open(fileOut.c_str(),ios::out);
  if (fff.is_open()) {   
fff<<"asID"<<"\tyear"<<"\tCountry"<<"\tOforShare"<< "\tAforestShare"<<"\tcab"<<"\tabFM"<<"\tabFMnew"<<"\tMAI_M3Ha"<<"\trotMAI";
fff<<"\t harvestThM3HaOld"<<"\t harvestFctM3HaOld"<<"\t harvestThM3HaNew"<<"\t harvestFctM3HaNew"<<"\tharvTotCur"<<"\tharvSW"<<"\tharvRW"<<"\tharvMAI"<<"\trotBiomass"<<"\tforNPV_CH"<<"\tfVal"<<"\t aval";
...
//**************************************************
  ...
  griddata harvestSWGrid = griddata(ResLongitude,ResLatitude,0);
  griddata harvestRWGrid = griddata(ResLongitude,ResLatitude,0);
...
cout<< "Start initialising cohorts"<< endl;
  initManagedForest(plots, species, &ffcov,&ffcoe, &ffdov, &ffdoe, dat_all, cohort_all, newCohort_all, 
	                                  maiForest, thinningForest, rotationType, managedForest, rotationForest, harvestSWGrid, harvestRWGrid, OforestShGrid);
...
//************************
    ...
    do {
      ...
      int Age = year-byear;
      if (year > refYear && PriceC>0) {priceC = PriceC * 44./12.;} 
      if (year > byear)
       {
cout << "Cprice= " << priceC << endl;
cout << "Adjusting FM .."<< endl;
       adjustManagedForest(plots, species, cohort_all, 
              ...
              rotationType, harvestSWGrid,harvestRWGrid, year, unmanaged,priceC);
       }
...

{
            calc(*it, species, *cohort_all[asID], *newCohort_all[asID], dat_all[asID], managedForest,
                 ...
                 harvestSWGrid, harvestRWGrid, year, priceC, asID, OforestShGrid);
...
//          if (it->PROTECT[2000]==0) {
          if ((it->PROTECT[2000]==0)&&((it->POTVEG[2000]<10 && it->FOREST[2000]>0 && it->MAIE[2000]>0)||(it->POTVEG[2000]<10 && (it->NPP[2000]>0)||it->MAIN[2000]>0))){                          
            if (regions.find(it->POLESREG[2000]) != regions.end()) { // Test only some regions
			  ...
			  if (settings.parametersMap.find("harvest_sawnW m3hayear") != settings.parametersMap.end()) {
                point.push_back("harvest_sawnW m3hayear");
				ASU.insert(dat_all[asID].simUnit, dat_all[asID].harvSawnWood, point);
                point.pop_back();
              } 
			  if (settings.parametersMap.find("harvest_restW m3hayear") != settings.parametersMap.end()) {
                point.push_back("harvest_restW m3hayear");
                ASU.insert(dat_all[asID].simUnit, dat_all[asID].harvRestWood, point);
                point.pop_back();
              } 
        ...
        if (years.find(year) != years.end()) {
          ...
          for (int countryCode = 1; countryCode < 244; countryCode++) {
            if (countriesList.find(countryCode) != countriesList.end()) {
              ...
              if (settings.parametersTable.find("harvest_demand_sawnw m3year") != settings.parametersTable.end()) {
                point.push_back("harvest_demand_sawnw m3year");
                obj.insert(CountryregWsawnprod.get(countryCode, year), point);				
                point.pop_back();
              }    
			   if (settings.parametersTable.find("harvest_demand_restw m3year") != settings.parametersTable.end()) {
                point.push_back("harvest_demand_restw m3year");
                obj.insert(CountryregWrestprod.get(countryCode, year), point);
                point.pop_back();
              }   
			    if (settings.parametersTable.find("harvest_sawnw m3year") != settings.parametersTable.end()) {
                point.push_back("harvest_sawnw m3year");
                obj.insert(CountriesSawnwoodHarvestM3Year.get(countryCode, year), point);
                point.pop_back();
              } 
				 if (settings.parametersTable.find("harvest_restw m3year") != settings.parametersTable.end()) {
                point.push_back("harvest_restw m3year");
                obj.insert(CountriesRestwoodHarvestM3Year.get(countryCode, year), point);
                point.pop_back();
              } 
...
////--------- 
	 ...
	 CountriesSawnwoodHarvestM3Year.printToFile("harvest_sawn_m3_year" + suffix, coeff.bYear, coeff.eYear, 1);
	 CountriesRestwoodHarvestM3Year.printToFile("harvest_rest_m3_year" + suffix, coeff.bYear, coeff.eYear, 1);
...
//------------------------------------------------------------------------------ 
     ...
     CountriesWSprod.printToFile("wprod_sawn_test" + suffix,coeff.bYear,coeff.eYear,1);
	 CountriesWRprod.printToFile("wprod_rest_test" + suffix,coeff.bYear,coeff.eYear,1);
//------------------------------------------------------------------------------     
