...
namespace g4m {
  ...
  public:
   ...
   //0..do only remove caused by stand density  to  1..do only typical removes
    ...
    ~ageStruct();
	//double HarvArea (int year,int country, );
	...
	double Biomass (int i);
	double BiomSdTab (int i);
...
//int	 TEST (double age_test, double mai_test, double bio_test, double rtime_test);				//OT
int	 TEST (double age_test, double mai_test);
    private:
    incrementTab *it;
    ffipol<double> *sws;
    ffipol<double> *hlv;
