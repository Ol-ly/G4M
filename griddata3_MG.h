...
//   Author:        Andriy Bun; modified by M & A 20.08.09
//   Date:          9.06.2008
...
//    double GridRows[];
  ...
  public:
      griddata(int HR, int VR, double val);
      griddata();
...
void griddata::PrintToFile(string fileName, string rastrType = "ESRI")
 {
  ofstream f;
  fileName = "out\\" + fileName + ".asc";
...
}else {cout<<"griddata3 error message: Specify correct rastrType: ESRI or GRASS"<<endl;}
//-----------------------------        
    for (int j = 0; j < VerResolution; j++)
     {
      for (int i = 0; i < HorResolution; i++)
...
void griddata::PrintToFilePrev(string fileName, string rastrType = "ESRI")
 {
  ofstream f;
  fileName = "out\\" + fileName + ".asc";
  f.open(fileName.c_str(),ios::out);
